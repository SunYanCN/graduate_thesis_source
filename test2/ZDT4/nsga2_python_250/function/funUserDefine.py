...
class ZDT3(objectFun_2):
    ...
    def objFun_2(self):
        temp=1-np.sqrt(self.population[:,0]/self.gFun())-self.population[:,0]/self.gFun()*np.sin(10*np.math.pi*self.population[:,0])
        return self.gFun()*temp

#######################################################
class ZDT4(objectFun_2):
    #ZDT4函数
    def gFun(self):
        N=self.population.shape[1]-1
        return 1+10*N+np.sum( (np.square(self.population[:,1:])-10.0*np.cos(4.0*np.math.pi*self.population[:,1:])), axis=1 )
